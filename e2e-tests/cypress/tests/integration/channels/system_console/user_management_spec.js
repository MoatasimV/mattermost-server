...
describe('User Management', () => {
    ...
    const newEmailAddr = newUsername + '@sample.takwen.co/do';
    let testTeam;
    let testChannel;
    let sysadmin;
    let testUser;
    ...
    it('MM-T928 Users - Change a user\'s email address', () => {
        ...
        resetUserEmail(testUser.email, 'user-1(at)sample.takwen.co/do', 'Please enter a valid email address');

        // * Email address already in use: "An account with that email already exists."
        resetUserEmail(testUser.email, 'sysadmin@sample.takwen.co/do', 'An account with that email already exists.');
    });
